...
$sql = "SELECT bookings.*,books.image as image FROM bookings INNER JOIN books ON bookings.book_id=books.id AND bookings.user_id!=books.user_id;";
$result = mysqli_query($conn, $sql);

if(isset($_GET['check'])){
    $id = $_GET['id'];
    $sql = "UPDATE bookings SET status='delivered' WHERE id=".$id;
    $result = mysqli_query($conn, $sql);
    return "true";
  }
...
<body class="">
              ...
              <div class="table-responsive" style="overflow:auto;">
                  <table class="table">
                    ...
                    do {
                      ...
                      <td>
                        ...
                        <td>
                          ...
                          <?php }else if($row[13] == "approved"){ ?>
                            <a onclick="receive('<?php echo $row[0]; ?>')">Received ?</a>                    
                          <?php }else{ ?>
                            <a>Deliverd</a>
                          <?php } ?>
                        </td>
                      ...
                      <?php $i++; }while ($row = $result -> fetch_row()); }else{ echo "No category found"; } ?>
                    </tbody>
                  </table>
                </div>
              </div>
  ...
  <script>
      function receive(id)
      {
          var log=confirm("Did you received your order ?");
          if(log==true)
          {
            $.ajax({
              type: "GET",
              url: 'http://localhost/biblio/outgoing.php?check=1&id='+id,
              success: function(data){
                if(data != "true"){
                  location.href="http://localhost/biblio/outgoing.php";
                }else{
                  location.href="http://localhost/biblio/outgoing.php";
                }
              }
